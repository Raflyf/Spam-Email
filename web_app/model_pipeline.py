...
# ==========================================
# KONSTANTA (sama dengan NB_XGB_MIX_IMPROVED.py)
# ==========================================
...
def preprocess(text):
    """Preprocessing teks email - identik dengan NB_XGB_MIX_IMPROVED.py"""
    text = str(text).lower()
...
def extra_features(raw_text):
    """Extra features - identik dengan NB_XGB_MIX_IMPROVED.py"""
    text    = str(raw_text)
    lower   = text.lower()
...
def find_best_threshold(proba, y_true):
    """Cari threshold optimal berdasarkan F1-Score weighted."""
...
class SpamPipeline:
    ...
    def _load_models(self):
        ...
        self.tfidf_word = joblib.load(TFIDF_WORD_PATH)
        self.tfidf_char = joblib.load(TFIDF_CHAR_PATH)
